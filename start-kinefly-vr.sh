...
start_vr() {
    ...
    echo -e "${YELLOW}⏳ Waiting for ${vr_name} topic...${NC}"
    timeout=30
    counter=0
    ...
    # Start ZMQ bridge
    echo -e "${GREEN}🌉 Starting ZMQ Bridge for ${vr_name} on port ${port}...${NC}"
    ...
    echo -e "${GREEN}✅ ${vr_name} running! (PID: ${kinefly_pid}, Bridge PID: ${bridge_pid}, Port: ${port})${NC}"
...
if [ -z "$VR_NUMBER" ]; then
    # Start all VRs
    echo -e "${BLUE}🚀 Starting all VRs (VR1-VR${MAX_VR})${NC}"
    for i in $(seq 1 $MAX_VR); do
        start_vr $i
        sleep 2  # Small delay between starts
    done
    echo -e "${GREEN}✅ All VRs started!${NC}"
else
    # Start specific VR
